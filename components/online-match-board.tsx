...
export function OnlineMatchBoard({ roomId, returnLobbyId, userId, isHost, isSpectator = false, status, state, stateVersion, hostDeck, guestDeck, timeLimitMinutes, startedAt, winnerUserId, endReason, hostRematchReady, guestRematchReady, initialLongPressMs = LONG_PRESS_DEFAULT_MS }: { roomId: string; returnLobbyId: string | null; userId: string; isHost: boolean; isSpectator?: boolean; status: string; state: Json; stateVersion: number; hostDeck: OnlineDeckSnapshot; guestDeck: OnlineDeckSnapshot; timeLimitMinutes: number; startedAt: string | null; winnerUserId: string | null; endReason: string | null; hostRematchReady: boolean; guestRematchReady: boolean; initialLongPressMs?: number }) {
