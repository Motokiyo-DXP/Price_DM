...
export function OnlineLobbySlots({ decks, lobbyId, myMatchIds, preferredDeckId, slots }: { decks: LobbyDeck[]; lobbyId: string; myMatchIds: string[]; preferredDeckId?: string | null; slots: LobbySlot[] }) {
  ...
  return <>
    ...
    {activeSlot ? <div className="online-dialog-backdrop" onPointerDown={(event) => { if (event.target === event.currentTarget) setActiveSlot(null); }}><form action={enterOnlineMatchSlotAction} className="match-entry-sheet"><input name="lobbyId" type="hidden" value={lobbyId} /><input name="slotId" type="hidden" value={activeSlot.id} /><header><div><p>MATCH {activeSlot.slot_number}</p><h2>マッチ受付の設定</h2></div><button aria-label="閉じる" onClick={() => setActiveSlot(null)} type="button">×</button></header><fieldset><legend>参加方法</legend><div className="match-role-switch"><label className={role === "player" ? "active" : ""}><input checked={role === "player"} disabled={activeSlot.player_count >= 2 && !myMatchIds.includes(activeSlot.game_room_id ?? "")} name="role" onChange={() => setRole("player")} type="radio" value="player" />対戦者</label><label className={role === "spectator" ? "active" : ""}><input checked={role === "spectator"} disabled={!activeSlot.game_room_id} name="role" onChange={() => setRole("spectator")} type="radio" value="spectator" />観戦者</label></div></fieldset><fieldset disabled={Boolean(activeSlot.game_room_id)}><legend>ルール</legend><label>フォーマット<select name="format" onChange={(event) => setFormat(event.target.value)} value={format}><option value="original">オリジナル</option><option value="advanced">アドバンス</option></select></label><label>時間制限<select defaultValue={activeSlot.time_limit_minutes} name="timeLimit"><option value="10">10分</option><option value="15">15分</option><option value="20">20分</option><option value="30">30分</option><option value="40">40分</option><option value="60">60分</option></select></label></fieldset>{activeSlot.game_room_id ? <><input name="format" type="hidden" value={activeSlot.format} /><input name="timeLimit" type="hidden" value={activeSlot.time_limit_minutes} /></> : null}{role === "player" ? <fieldset><legend>使用するデッキ</legend><select defaultValue={defaultDeckId} key={`${activeSlot.id}-${format}`} name="deckId" required>{matchingDecks.map((deck) => <option key={deck.id} value={deck.id}>{deck.name}</option>)}</select>{matchingDecks.length === 0 ? <p>このフォーマットで使える40枚デッキがありません。</p> : null}</fieldset> : <input name="deckId" type="hidden" value="" />}<button className="button" disabled={role === "player" && matchingDecks.length === 0} type="submit">{role === "spectator" ? "観戦する" : "この受付に参加"}</button></form></div> : null}
  </>;
}
