create or replace function private.changed_game_card_ids(p_before jsonb, p_after jsonb)
returns text[]
language sql
immutable
security invoker
set search_path = ''
as $$
  with before_cards as (
    select
      card.value ->> 'instanceId' as instance_id,
      pg_catalog.jsonb_build_object(
        'owner', player.key,
        'zone', zone.key,
        'position', card.ordinality,
        'card', card.value
      ) as signature
    from pg_catalog.jsonb_each(coalesce(p_before -> 'players', '{}'::jsonb)) as player
    cross join pg_catalog.jsonb_each(player.value) as zone
    cross join lateral pg_catalog.jsonb_array_elements(zone.value) with ordinality as card(value, ordinality)
  ),
  after_cards as (
    select
      card.value ->> 'instanceId' as instance_id,
    ...
    from pg_catalog.jsonb_each(coalesce(p_after -> 'players', '{}'::jsonb)) as player
    cross join pg_catalog.jsonb_each(player.value) as zone
    cross join lateral pg_catalog.jsonb_array_elements(zone.value) with ordinality as card(value, ordinality)
  )
  select coalesce(pg_catalog.array_agg(coalesce(before_cards.instance_id, after_cards.instance_id))
    filter (where coalesce(before_cards.instance_id, after_cards.instance_id) is not null), '{}'::text[])
  from before_cards
  full join after_cards using (instance_id)
  where before_cards.signature is distinct from after_cards.signature
$$;

alter function public.update_game_room_state(uuid, bigint, jsonb) set schema private;
alter function private.update_game_room_state(uuid, bigint, jsonb) rename to update_game_room_state_legacy;
drop function private.update_game_room_state_legacy(uuid, bigint, jsonb);

create function private.update_game_room_state_impl(p_room_id uuid, p_expected_version bigint, p_state jsonb)
...
revoke all on function private.changed_game_card_ids(jsonb, jsonb) from public, anon, authenticated;
...
create function public.update_game_room_state(p_room_id uuid, p_expected_version bigint, p_state jsonb)
returns table(state jsonb, state_version bigint)
language sql
security invoker
set search_path = ''
as $$ select * from private.update_game_room_state_impl(p_room_id, p_expected_version, p_state) $$;
