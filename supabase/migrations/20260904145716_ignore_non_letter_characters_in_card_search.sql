...
comment on function public.normalize_card_search(text) is
  'Applies NFKC, folds katakana to hiragana, lowercases Latin, and removes every character except Latin letters, hiragana, katakana, and kanji.';
...
where duplicate.canonical_card_id = retained.canonical_card_id
  ...
  and public.normalize_card_search(duplicate.term)
      = public.normalize_card_search(retained.term)
  and (
