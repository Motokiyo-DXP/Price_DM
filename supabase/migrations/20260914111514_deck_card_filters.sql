create function private.deck_filter_options_impl()
returns jsonb
...
$$;
revoke all on function private.deck_filter_options_impl() from public, anon;
grant execute on function private.deck_filter_options_impl() to authenticated;

create function public.deck_filter_options()
returns jsonb language sql stable security invoker set search_path = ''
as $$ select private.deck_filter_options_impl(); $$;
revoke all on function public.deck_filter_options() from public, anon;
grant execute on function public.deck_filter_options() to authenticated;

create function private.search_deck_cards_filtered_impl(
...
begin
  ...
  order by
    ...
    case when p_sort = 'relevance' then m.exact end desc,
    case when p_sort = 'relevance' then m.prefix end desc,
    case when p_sort = 'relevance' then m.contains end desc,
    case when p_sort = 'relevance' then m.score end desc,
    case when p_sort = 'name' and p_ascending then m.name end asc,
    case when p_sort = 'name' and not p_ascending then m.name end desc,
    case when p_sort = 'prints' and p_ascending then m.prints end asc,
    case when p_sort = 'prints' and not p_ascending then m.prints end desc,
    m.name, m.id
...
as $$
  select * from private.search_deck_cards_filtered_impl(
    p_query, p_limit, p_sort, p_ascending, p_product_name, p_card_number,
    p_civilizations, p_civilization_mode, p_color, p_card_types,
    p_min_cost, p_max_cost, p_no_cost, p_image
  );
$$;
