  ...
  foreach v_player in array array['p1','p2'] loop
    foreach v_zone in array v_zones loop
      ...
      for v_card in select value from pg_catalog.jsonb_array_elements(p_submitted #> array['players',v_player,v_zone]) loop
        ...
        v_card := v_card || pg_catalog.jsonb_build_object(
        ...
        if v_zone = 'reveal' then
          -- A card newly entering the temporary zone always starts unpublished.
          if not exists (
            select 1 from pg_catalog.jsonb_array_elements(coalesce(p_authoritative #> array['players',v_player,'reveal'],'[]'::jsonb)) prior(card)
            where prior.card ->> 'instanceId' = v_card ->> 'instanceId'
          ) then
            v_card := pg_catalog.jsonb_set(v_card,'{revealPublished}','false'::jsonb,true);
          end if;
        else
    ...
    foreach v_zone in array v_zones loop
      ...
      for v_card in select value from pg_catalog.jsonb_array_elements(coalesce(p_state #> array['players',v_player,v_zone],'[]'::jsonb)) loop
        v_visible := case when v_zone = 'reveal'
          then p_viewer = v_player or v_card ->> 'revealPublished' = 'true'
