...
create temporary table dm26_product (id bigint primary key) on commit drop;
...
select game.id, 'DM26-RP3', 'DM26-RP3 デュエル・マスターズTCG 逆札篇 第3弾 勝敵！逆転おこせ鬼の∞ディスペクター!!', date '2026-09-19', 'day', 'https://dm.takaratomy.co.jp/product/dm26rp3/', pg_catalog.now()
from public.tcg_games game where game.slug = 'duel-masters'
on conflict (game_id, product_code) do update
set product_name = excluded.product_name, release_date = excluded.release_date,
    release_date_precision = excluded.release_date_precision, official_url = excluded.official_url,
    source_checked_at = excluded.source_checked_at, updated_at = pg_catalog.now()
where public.card_products.product_name is distinct from excluded.product_name
   or public.card_products.release_date is distinct from excluded.release_date
   or public.card_products.release_date_precision is distinct from excluded.release_date_precision
   or public.card_products.official_url is distinct from excluded.official_url;
insert into dm26_product(id)
select products.id from public.card_products products
join public.tcg_games game on game.id = products.game_id
where game.slug = 'duel-masters' and products.product_code = 'DM26-RP3';
create temporary table merge_names (
  survivor_name text primary key,
  merged_name text unique not null
) on commit drop;
insert into merge_names values
('時の法皇 ミラダンテⅫ','時の法皇 ミラダンテXII'),
('ツクっちょ <メイ様.Star>','ツクっちょ ＜メイ様.Star＞'),
('カツラデランス / 「アフロ行きま～す!!」','カツラデランス / 「アフロ行きま～す！！」'),
('「正義星帝」 <ライオネル.Star>','「正義星帝」 ＜ライオネル.Star＞'),
('「正義星帝」 <鬼羅.Star>','「正義星帝」 ＜鬼羅.Star＞'),
('オンソク童子 <ターボ.鬼>','オンソク童子 ＜ターボ.鬼＞'),
('テラ・スザーク <ロマノフ.Star>','テラ・スザーク ＜ロマノフ.Star＞'),
('めっちゃ！デンヂャラスG３ / ケッシング・ゼロ','めっちゃ！デンヂャラスG3 / ケッシング・ゼロ'),
('キャンベロ <レッゾ.Star>','キャンベロ ＜レッゾ.Star＞'),
('ルピア&ガ:ナテハ','ルピア＆ガ：ナテハ'),
('ミラクル１ ドレミ24','ミラクル1 ドレミ24'),
('クイーン&かぼちゃうちゃう','クイーン＆かぼちゃうちゃう'),
('カチコミ入道 <バトライ.鬼>','カチコミ入道 ＜バトライ.鬼＞'),
('モンキッド ＜ライゾウ.Star＞','モンキッド <ライゾウ.Star>'),
('爆殺!! 覇悪怒楽苦','爆殺！！ 覇悪怒楽苦'),
('大邪眼Ｂ・ロマノフ','大邪眼B・ロマノフ'),
('戦略のＤ・Ｈ アツト','戦略のD・H アツト'),
('勇者の１号 ハムカツマン蒼','勇者の1号 ハムカツマン蒼'),
('「楯騎士」 ＜アブソリュート.Star＞','「楯騎士」 <アブソリュート.Star>'),
('ゴルドーザ ＜ドラギリア.Star＞','ゴルドーザ <ドラギリア.Star>'),
('コバンザ ＜ジャニット.Star＞','コバンザ <ジャニット.Star>'),
('ダチッコ ＜マック.Star＞','ダチッコ <マック.Star>'),
('ＪＫ大バクチ','JK大バクチ'),
('ＪＫパーネイチャー','JKパーネイチャー'),
('ＪＫ軍曹チョキパン','JK軍曹チョキパン'),
('ジョニー＆ジョラゴン 〜Ｊの意志〜','ジョニー＆ジョラゴン 〜Jの意志〜'),
('ドッカン！ ゴートッＱ','ドッカン！ ゴートッQ'),
('爆裂B-BOY','爆裂Ｂ-ＢＯＹ'),
('アクア・ジェット ＜突撃・ブラザー！＞','アクア・ジェット <突撃・ブラザー！>'),
('超電磁ヘルラッシュM','超電磁ヘルラッシュＭ'),
('サイバーX・ザナドゥ','サイバーＸ・ザナドゥ'),
('キサナティック・X','キサナティック・Ｘ'),
('頂上混成 ガリュディアス・モモミーズ’22','頂上混成 ガリュディアス・モモミーズ''22');

create temporary table merge_pairs on commit drop as
select survivor.id as survivor_id, merged.id as merged_id
from merge_names names
join public.canonical_cards survivor
  on survivor.name = names.survivor_name and survivor.deleted_at is null
join public.canonical_cards merged
  on merged.name = names.merged_name and merged.deleted_at is null
where survivor.id <> merged.id;
...
begin
  if exists (
    select 1
    from merge_names names
    where (select count(*) from public.canonical_cards cards where cards.deleted_at is null and cards.name in (names.survivor_name, names.merged_name)) = 1
      and not exists (select 1 from public.canonical_cards cards where cards.deleted_at is null and cards.name = names.survivor_name)
  ) then raise exception 'DM26-RP3 merge survivor is missing'; end if;
  if exists (
    select 1 from merge_pairs pairs
    join public.canonical_cards survivor on survivor.id = pairs.survivor_id
    join public.canonical_cards merged on merged.id = pairs.merged_id
    where survivor.manually_locked or merged.manually_locked
       or exists (
         select 1 from public.deck_cards source
         join public.deck_cards target on target.deck_id = source.deck_id and target.zone = source.zone
  ...
  ) then raise exception 'DM26-RP3 merge preflight failed'; end if;
end;
$$;

insert into public.card_search_terms(canonical_card_id, term, normalized_term, term_kind, source, verified, priority)
select pairs.survivor_id, terms.term, terms.normalized_term, terms.term_kind, terms.source, terms.verified, terms.priority
from merge_pairs pairs join public.card_search_terms terms on terms.canonical_card_id = pairs.merged_id
on conflict (canonical_card_id, normalized_term, term_kind) do nothing;
delete from public.card_search_terms terms using merge_pairs pairs where terms.canonical_card_id = pairs.merged_id;
update public.card_prints prints set canonical_card_id = pairs.survivor_id, updated_at = pg_catalog.now() from merge_pairs pairs where prints.canonical_card_id = pairs.merged_id;
update public.deck_cards cards set canonical_card_id = pairs.survivor_id, updated_at = pg_catalog.now() from merge_pairs pairs where cards.canonical_card_id = pairs.merged_id;
update public.decks decks set icon_canonical_card_id = pairs.survivor_id, updated_at = pg_catalog.now() from merge_pairs pairs where decks.icon_canonical_card_id = pairs.merged_id;
update public.account_card_bookmarks bookmarks set canonical_card_id = pairs.survivor_id from merge_pairs pairs where bookmarks.canonical_card_id = pairs.merged_id;
update public.price_records records set canonical_card_id = pairs.survivor_id, updated_at = pg_catalog.now() from merge_pairs pairs where records.canonical_card_id = pairs.merged_id;
update public.canonical_cards merged set deleted_at = pg_catalog.now(), updated_at = pg_catalog.now() from merge_pairs pairs where merged.id = pairs.merged_id;
...
join public.card_prints prints on prints.product_id = (select id from dm26_product)
  and prints.deleted_at is null and prints.official_card_id = source.official_card_id
union all
select source.official_card_id, prints.id as card_print_id
from official_source source
join public.card_prints prints on prints.product_id = (select id from dm26_product)
  and prints.deleted_at is null and prints.card_number = source.card_number
where not exists (
  select 1 from public.card_prints exact_print
  where exact_print.product_id = (select id from dm26_product)
    and exact_print.deleted_at is null and exact_print.official_card_id = source.official_card_id
);

do $$
begin
  if (select count(*) from existing_print_matches) not in (0, 80, 156)
...
where cards.id = metadata.canonical_card_id and not cards.manually_locked
  and (cards.name, cards.source_name, cards.cost, cards.civilizations, cards.card_types)
      is distinct from (metadata.name, metadata.name, metadata.cost, metadata.civilizations, metadata.card_types);
...
where prints.id = matches.card_print_id
  and (prints.official_card_id, prints.card_number, prints.product_name, prints.official_url)
      is distinct from (source.official_card_id, source.card_number,
        'DM26-RP3 デュエル・マスターズTCG 逆札篇 第3弾 勝敵！逆転おこせ鬼の∞ディスペクター!!', source.official_url);
...
select source_canonicals.canonical_card_id, source.official_card_id, source.card_number,
  'DM26-RP3 デュエル・マスターズTCG 逆札篇 第3弾 勝敵！逆転おこせ鬼の∞ディスペクター!!', source.official_url, pg_catalog.now(), (select id from dm26_product)
...
insert into public.card_search_terms as terms(canonical_card_id, term, normalized_term, term_kind, source, verified, priority)
...
do $$
begin
  if (select count(*) from public.card_prints where product_id = (select id from dm26_product) and official_card_id in (select official_card_id from official_source) and deleted_at is null) <> 156 then
    raise exception 'DM26-RP3 official print validation failed';
  end if;
  if exists (
    select 1 from public.deck_cards cards
    left join public.canonical_cards canonical on canonical.id = cards.canonical_card_id
    where canonical.id is null
  ) or exists (
    select 1 from public.account_card_bookmarks bookmarks
    left join public.canonical_cards canonical on canonical.id = bookmarks.canonical_card_id
    where canonical.id is null
  ) or exists (
    select 1 from public.price_records records
    left join public.canonical_cards canonical on canonical.id = records.canonical_card_id
    where canonical.id is null
  ) then raise exception 'DM26-RP3 canonical reference validation failed'; end if;
end;
$$;
commit;
