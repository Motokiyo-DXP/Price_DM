create or replace function private.game_card_map(p_state jsonb)
returns jsonb
language sql immutable security invoker set search_path = ''
as $$
  select coalesce(pg_catalog.jsonb_object_agg(card ->> 'instanceId', card), '{}'::jsonb)
  from pg_catalog.jsonb_path_query(p_state, '$.players.*.*[*]') as card
  where nullif(card ->> 'instanceId', '') is not null
$$;
...
declare
  ...
  v_authoritative_unique_count integer;
  v_submitted_unique_count integer;
begin
  if pg_catalog.jsonb_typeof(p_submitted -> 'players') <> 'object' then
    raise exception 'invalid_game_state' using errcode = '22023';
  end if;
  select pg_catalog.count(*)::integer into v_submitted_count
  from pg_catalog.jsonb_path_query(p_submitted, '$.players.*.*[*]');
  ...
  if v_submitted_count <> v_authoritative_unique_count
     ...
     or exists (
       select 1 from pg_catalog.jsonb_object_keys(v_submitted_map) as submitted_id
       where not (v_authoritative_map ? submitted_id)
     ) then
    ...
    foreach v_zone in array v_zones loop
      ...
      for v_card in select value from pg_catalog.jsonb_array_elements(p_submitted #> array['players', v_player, v_zone]) loop
        ...
        v_cards := v_cards || pg_catalog.jsonb_build_array(v_card || pg_catalog.jsonb_build_object(
          'canonicalCardId', v_original -> 'canonicalCardId',
          'name', v_original -> 'name',
          'imageUrl', v_original -> 'imageUrl',
          'cost', v_original -> 'cost',
          'civilizations', v_original -> 'civilizations',
          'cardTypes', v_original -> 'cardTypes'
        ));
      end loop;
  ...
  foreach v_player in array array['p1','p2'] loop
    foreach v_zone in array v_zones loop
      ...
      for v_card in select value from pg_catalog.jsonb_array_elements(coalesce(p_state #> array['players', v_player, v_zone], '[]'::jsonb)) loop
        v_visible := v_zone <> 'deck' and (
          p_spectator
          or v_card ->> 'face' = 'face_up'
          or (v_card ->> 'face' = 'owner_only' and p_viewer = v_player)
          or (p_state -> 'inspection' ->> 'cardId' = v_card ->> 'instanceId'
              and p_state -> 'inspection' ->> 'viewer' = p_viewer)
        );
        if not coalesce(v_visible, false) then
          v_card := (v_card - array['canonicalCardId','name','imageUrl','cost','civilizations','cardTypes'])
            || pg_catalog.jsonb_build_object('canonicalCardId', null, 'name', '非公開カード', 'imageUrl', null,
                 'cost', null, 'civilizations', '[]'::jsonb, 'cardTypes', '[]'::jsonb);
        end if;
...
revoke all on function private.game_card_map(jsonb) from public, anon, authenticated;
