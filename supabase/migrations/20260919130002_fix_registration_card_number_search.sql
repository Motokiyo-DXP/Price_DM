-- Preserve card-number and product-name contains search when normalization
-- removes digits (for example, RP3 becomes rp for card-term matching).
...
as $$
  ...
  short_print_match_rows as materialized (
    ...
    cross join search_input as input
    where pg_catalog.char_length(input.raw_query) between 1 and 2
  ...
  long_print_match_rows as materialized (
    ...
    from public.card_prints as prints
    cross join search_input as input
    where pg_catalog.char_length(input.raw_query) >= 3
      and input.raw_query <> ''
      and prints.deleted_at is null
