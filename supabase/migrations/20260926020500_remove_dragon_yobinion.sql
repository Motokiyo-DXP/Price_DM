drop function if exists public.run_game_yobinion(uuid,bigint,text,text,boolean);
drop function if exists private.run_game_yobinion_impl(uuid,bigint,text,text,boolean);

create function private.run_game_yobinion_impl(
  p_room_id uuid,
...
revoke all on function private.run_game_yobinion_impl(uuid,bigint,text,text) from public,anon,authenticated;
grant execute on function private.run_game_yobinion_impl(uuid,bigint,text,text) to authenticated;

create function public.run_game_yobinion(
  p_room_id uuid,p_expected_version bigint,p_owner text,p_source_id text
)
returns table(state jsonb,state_version bigint,found boolean)
language sql security invoker set search_path = ''
as $$
  select private.redact_game_room_state_for_current_user(p_room_id,result.state),result.state_version,result.found
  from private.run_game_yobinion_impl(p_room_id,p_expected_version,p_owner,p_source_id) result
$$;

revoke all on function public.run_game_yobinion(uuid,bigint,text,text) from public,anon;
grant execute on function public.run_game_yobinion(uuid,bigint,text,text) to authenticated;
