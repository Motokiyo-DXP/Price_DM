-- Add offset pagination without regressing the current release-date sorting contract.
create function private.search_deck_cards_filtered_paged_impl(
  p_query text default '', p_limit integer default 30, p_offset integer default 0,
...
revoke all on function private.search_deck_cards_filtered_paged_impl(
  text,integer,integer,text,boolean,text,text,text[],text,text,text[],integer,integer,boolean,text
) from public, anon;
grant execute on function private.search_deck_cards_filtered_paged_impl(
  text,integer,integer,text,boolean,text,text,text[],text,text,text[],integer,integer,boolean,text
) to authenticated;

-- Replace the previous public signature so PostgREST exposes one unambiguous RPC.
...
as $$
  select * from private.search_deck_cards_filtered_paged_impl(
    p_query,p_limit,p_offset,p_sort,p_ascending,p_product_name,p_card_number,
    p_civilizations,p_civilization_mode,p_color,p_card_types,p_min_cost,
    p_max_cost,p_no_cost,p_image
  );
$$;
