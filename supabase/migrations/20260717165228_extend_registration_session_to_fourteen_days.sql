-- Keep the shared registration PIN session for fourteen days. Only opaque
-- token hashes are stored in Postgres; the raw PIN is never stored in cookies.
...
begin
  ...
  v_expires_at := pg_catalog.clock_timestamp() + interval '14 days';
...
comment on function public.create_registration_session(text) is
  'Verifies the shared PIN and returns a one-time-visible opaque token valid for 14 days.';
