-- The historical authentication helper resolves JWT claims through the
-- protected auth schema. This SECURITY DEFINER function is owned by
-- price_registration_executor, which
-- intentionally has no auth schema privileges. Read the same request GUCs
-- directly so authenticated requests remain PIN-free without widening access.
