...
import { RoomDeckSelect, RoomReadyButton } from "@/components/room-deck-ready-form";
...
export default async function RoomPage({ params, searchParams }: { params: Promise<{ roomId: string }>; searchParams: Promise<{ error?: string }> }) {
  ...
  const [{ data: room, error }, { data: deckLabels }, { data: decks }] = await Promise.all([
    supabase.from("game_rooms").select("id, room_code, host_user_id, guest_user_id, status, format, deck_is_public, host_ready, guest_ready, state_version, time_limit_minutes, started_at, winner_user_id, end_reason, host_rematch_ready, guest_rematch_ready").eq("id", roomId).maybeSingle(),
  ...
  const playableDeckChoices = playableDecks.map((deck) => {
    ...
    const imageKey = (iconCard?.card_print_id ? orderedIconPrints.find((print) => print.id === iconCard.card_print_id)?.image_key : null) ?? orderedIconPrints.find((print) => print.canonical_card_id === iconId)?.image_key;
    return { id: deck.id, name: deck.name, imageUrl: getCardImageUrl(imageKey) };
  });
  const isDeckPublic = room.deck_is_public;
  const hostDeckImageUrl = isDeckPublic ? getCardImageUrl(labels?.host_icon_image_key) ?? (isHost ? selectedDeckImageUrl : null) : null;
  const guestDeckImageUrl = isDeckPublic ? getCardImageUrl(labels?.guest_icon_image_key) ?? (isGuest ? selectedDeckImageUrl : null) : null;
  ...
  return <section className="room-page">
    ...
    <div className="seat-grid">
      <article className={isHost ? "my-seat" : ""}><header><span>{isHost ? "自分（01）" : "対戦相手（01）"}</span><strong className={room.host_ready ? "ready" : "waiting"}>{room.host_ready ? "準備完了" : "準備中"}</strong></header><div className="room-player-details"><span aria-label={`${hostProfile?.display_name ?? "ホスト"}のアカウントアイコン`} className={`room-player-avatar ${hostProfile?.avatar_url ? "has-image" : ""}`} style={hostProfile?.avatar_url ? { backgroundImage: `url("${hostProfile.avatar_url}")` } : undefined}>{hostProfile?.avatar_url ? null : (hostProfile?.display_name ?? (isHost ? "自分" : "相手")).slice(0, 1).toUpperCase()}</span>{isHost ? <RoomDeckSelect decks={playableDeckChoices} myReady={myReady} roomId={room.id} selectedDeckId={selectedDeckId} /> : null}{isDeckPublic ? <CardArtwork className="room-deck-artwork" imageUrl={hostDeckImageUrl} name={`${hostDeck.name}のアイコン`} sizes="62px" /> : <span aria-label="非公開のデッキアイコン" className="room-deck-emblem">?</span>}<span><h2>{hostDeck.name}</h2><p>{isDeckPublic ? `${room.format === "advanced" ? "アドバンス" : "オリジナル"}・40枚` : "デッキは非公開です"}</p></span></div></article>
      <article className={isGuest ? "my-seat" : ""}><header><span>{isGuest ? "自分（02）" : "対戦相手（02）"}</span><strong className={room.guest_ready ? "ready" : "waiting"}>{room.guest_ready ? "準備完了" : "準備中"}</strong></header>{guestDeck ? <div className="room-player-details"><span aria-label={`${guestProfile?.display_name ?? "ゲスト"}のアカウントアイコン`} className={`room-player-avatar ${guestProfile?.avatar_url ? "has-image" : ""}`} style={guestProfile?.avatar_url ? { backgroundImage: `url("${guestProfile.avatar_url}")` } : undefined}>{guestProfile?.avatar_url ? null : (guestProfile?.display_name ?? (isGuest ? "自分" : "相手")).slice(0, 1).toUpperCase()}</span>{isGuest ? <RoomDeckSelect decks={playableDeckChoices} myReady={myReady} roomId={room.id} selectedDeckId={selectedDeckId} /> : null}{isDeckPublic ? <CardArtwork className="room-deck-artwork" imageUrl={guestDeckImageUrl} name={`${guestDeck.name}のアイコン`} sizes="62px" /> : <span aria-label="非公開のデッキアイコン" className="room-deck-emblem">?</span>}<span><h2>{guestDeck.name}</h2><p>{isDeckPublic ? `${room.format === "advanced" ? "アドバンス" : "オリジナル"}・40枚` : "デッキは非公開です"}</p></span></div> : <div className="room-seat-empty"><span aria-hidden="true" className="room-player-avatar">?</span><span><h2>参加者を待っています</h2><p>ルームIDを対戦相手へ共有してください。</p></span></div>}</article>
    </div>
    ...
    {!isSpectator && lobbyOpen && selectedDeckId ? <RoomReadyButton key={`${selectedDeckId}-${myReady}`} myReady={myReady} roomId={room.id} selectedDeckId={selectedDeckId} /> : null}
  </section>;
}
