...
test("MAXを含む新しい山札閲覧は山札順から始まり、コスト表示は明示切替だけで使う", () => {
  const board = readFileSync(new URL("../components/playtest-board.tsx", import.meta.url), "utf8");
  assert.match(board, /const nextInspection: DeckInspection = \{ cards: inspectedCards,[^;]*mode: "deck"/);
  assert.match(board, /setDeckInspection\(beginDeckInspectionSession\(nextInspection\)\)/);
  assert.match(board, /if \(inspection\.mode === "deck"\) return inspection\.cards/);
  ...
  assert.match(close, /else\s*\{\s*setDeckInspection\(\(current\) => current \? beginDeckInspectionSession\(current\) : current\);\s*\}/);
  assert.match(close, /if \(inspection\?\.maxSession\)[\s\S]*setDeckInspection\(null\)/);
  assert.match(close, /const top = topIds\.flatMap[\s\S]*const bottom = bottomIds\.flatMap[\s\S]*const remaining = shuffleCards[\s\S]*deck: \[\.\.\.top, \.\.\.remaining, \.\.\.bottom\]/);
});
...
test("山札閲覧カードの挿入markerは閲覧ゾーン内の並び替え先だけに表示する", () => {
  ...
  assert.match(board, /if \(viewer\) \{[\s\S]*viewer\.contains\(element\)[\s\S]*targetCard\?\.classList\.add\("deck-insert-before"\)/);
  assert.match(board, /function moveDeckViewerCard\([\s\S]*moveCardsBetweenZones\(current, owner, "deck", to, new Set\(ids\)/);
  assert.match(board, /function moveDeckViewerCard\([\s\S]*removeInspectedCards\(ids\)/);
});

test("山札閲覧中の短いタップは既存の補助ゾーントグルで閉じ、ドローしない", () => {
  const board = readFileSync(new URL("../components/playtest-board.tsx", import.meta.url), "utf8");
  assert.match(board, /function DeckPile\(\{ active,/);
  assert.match(board, /if \(active\) onSelectAuxiliaryZone\(owner, "deck"\);\s+else onDraw\(owner\);/);
  assert.match(board, /<DeckPile active=\{activeAuxiliaryZone === "deck"\}/);
});
