...
test("重ね先カード上の保持は300msから進捗表示し1000msでパネルを開く", () => {
  assert.equal(STACK_HOLD_PROGRESS_MS, 300);
  assert.equal(STACK_HOLD_MENU_MS, 1000);
  assert.equal(stackHoldPhase(299), "drag");
  assert.equal(stackHoldPhase(300), "progress");
  assert.equal(stackHoldPhase(999), "progress");
  assert.equal(stackHoldPhase(1000), "menu");
});
