...
import { advanceTurn, bundleSelectedCards, changeSlayerCount, clearCardMarkers, countZoneCards, detachCardFromStack, drawRandomCard, findAttachedSpreadStackId, flipStackCards, initialBoard, isMulticolorCard, moveCardsBetweenZones, toggleLegacyCardTapState, toggleRevealPublic, resetBoard, resolveCardCivilizations, resolveDraggedCardIds, runYobinion, setCardMarker, shuffleSelectedCards, stackOnHorizontalRoot, shuffleStackCards, unbundleStack, untapAllCards, untapZoneCards } from "./playfield-board.ts";
...
test("山札閲覧状態は盤面状態に含まれ、オンライン同期で同じ対象・順序を再現できる", () => {
  const board = initialBoard(cards, () => 0.5);
  assert.equal(board.deckInspection, null);
  board.deckInspection = { count: "max", order: "cost", owner: "p1", viewer: "p1" };
  assert.deepEqual(structuredClone(board).deckInspection, board.deckInspection);
});
