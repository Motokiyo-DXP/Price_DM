...
test("ドラッグ保持は対象カードごとに計時し250msで進捗、500msで共通Mayaパネルを開く", () => {
  ...
  assert.ok(board.includes("<StackHoldProgress meter={stackHoldProgress} />"));
  assert.ok(board.includes("totalDurationMs: STACK_HOLD_MENU_MS"));
...
test("引用ゾーンは種類にかかわらず外側を広げずカード列だけ横スクロールする", () => {
  assert.ok(!board.includes('className={`battle-auxiliary-drawer ${activeAuxiliaryZone === "deck"'));
...
test("山札閲覧はauxiliary drawer外の専用slotに表示し、横スクロールと余白を保つ", () => {
  assert.ok(board.includes("<div className=\"battle-self-shield-row\">"));
  assert.match(board, /<\/div>\{deckInspection\}<Zone \{\.\.\.zoneProps\} cards=\{board\.players\[owner\]\.mana\} zone="mana"/);
  ...
  assert.ok(css.includes('.deck-viewer .deck-view-shuffle>.deck-view-shuffle-icon{font-size:32px'));
  assert.ok(css.includes('@media(max-width:700px){.deck-viewer .deck-view-shuffle>.deck-view-shuffle-icon{font-size:38px'));
});
