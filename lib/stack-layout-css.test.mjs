...
test("安全な山札閲覧は旧最新版の数値ホイールと専用drawer表示を再利用する", () => {
  ...
  assert.ok(board.includes('className="deck-inspection-header"'));
  assert.ok(board.includes('className="deck-view-mode-toggle"'));
  assert.ok(css.includes('.deck-view-number-wheel{height:112px'));
  assert.ok(css.includes('.deck-view-drawer>.deck-inspection-zone{'));
  assert.ok(css.includes('.deck-inspection-zone>.play-zone-cards::after{content:"";flex:0 0 30%'));
});
