...
test("全角数字を半角へ変換し、数字以外を除去して9桁に制限する", () => {
  assert.equal(normalizePriceInput("１２３４円"), "1234");
  assert.equal(normalizePriceInput("￥ 1,980-"), "1980");
  assert.equal(normalizePriceInput("１２３４５６７８９０"), "123456789");
  assert.equal(normalizePriceInput("価格なし"), "");
});
