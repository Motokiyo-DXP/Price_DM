...
test("両対戦者の準備完了でホスト操作を待たず自動開始する", () => {
  ...
  assert.ok(roomSource.includes("対戦者2名が準備完了すると、自動的に対戦が始まります。"));
...
test("準備完了の取り消し時も選択済みデッキIDを送信する", () => {
  const roomSource = readFileSync(new URL("../app/rooms/[roomId]/page.tsx", import.meta.url), "utf8");
  assert.ok(roomSource.includes('{myReady ? <input name="deckId" type="hidden" value={selectedDeckId} /> : null}'));
});
...
test("準備完了後は待機画面の下ではなく対戦専用ページへ移動する", () => {
  ...
  assert.ok(roomsSource.includes('/battle` : `/rooms/'));
});
