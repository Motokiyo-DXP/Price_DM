...
test("山札確認は縦ダイヤルとMAXを排他利用し、前回枚数と表示順をローカル状態だけで保持する", () => {
  ...
  assert.ok(boardSource.includes('useState<number | "max">(3)'));
  assert.ok(boardSource.includes('useRef<Record<PlayerId, number>>({ p1: 3, p2: 3 })'));
  assert.ok(boardSource.includes('onUseDial={() => setDeckViewCount((current) => current === "max" ? lastDeckViewCount.current[deckViewConfirm] : current)}'));
  assert.ok(boardSource.includes('onClick={() => setDeckViewCount("max")}'));
  assert.ok(boardSource.includes('order: count === "max" ? "cost" : "deck"'));
  assert.ok(boardSource.includes('if (inspection.order === "deck") return inspection.cards'));
});
