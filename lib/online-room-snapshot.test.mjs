...
test("準備完了の取り消し時も選択済みデッキIDを送信する", () => {
  const readyFormSource = readFileSync(new URL("../components/room-deck-ready-form.tsx", import.meta.url), "utf8");
  assert.ok(readyFormSource.includes('<input name="deckId" type="hidden" value={deckId} />'));
});

test("待機ルームのデッキ変更は準備完了とは別に即時snapshot更新を送信する", () => {
  ...
  assert.ok(readyFormSource.includes("event.currentTarget.form?.requestSubmit()"));
