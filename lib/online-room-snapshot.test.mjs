...
test("オンラインの全ヨビニオン起動経路は非公開山札をローカル検索せずサーバーへ委譲する", () => {
  ...
  assert.ok(boardSource.includes("function executeYobinion(owner: PlayerId, sourceId: string)"));
  assert.ok(boardSource.includes("onYobinionRequest({ owner, sourceId })"));
  assert.ok(boardSource.includes("executeYobinion(owner, cardId)"));
  assert.ok(boardSource.includes("executeYobinion(markingMenu.owner, markingMenu.card.instanceId)"));
  assert.doesNotMatch(boardSource, /dragon.?yobinion|dragonOnly|dragon: true/i);
  assert.doesNotMatch(onlineSource, /p_dragon_only|dragonOnly/i);
...
test("オンライン山札閲覧は実zoneへ一度だけ移し、相手向けstateはinspectionのidentityを隠す", () => {
  const boardSource = readFileSync(new URL("../components/playtest-board.tsx", import.meta.url), "utf8");
  const onlineSource = readFileSync(new URL("../components/online-match-board.tsx", import.meta.url), "utf8");
  const boardStateSource = readFileSync(new URL("./playfield-board.ts", import.meta.url), "utf8");
  const migration = readFileSync(new URL("../supabase/migrations/20260923145311_real_deck_inspection_zone.sql", import.meta.url), "utf8");
  assert.ok(boardSource.includes("onDeckInspectionRequest?: (request: ServerDeckInspectionRequest) => Promise<CardInstance[] | null>"));
  assert.ok(boardSource.includes("commit((current) => inspectDeckCards(current, pending.owner"));
  assert.ok(boardSource.includes('moveCardsBetweenZones(current, owner, "deckInspection", to'));
  assert.ok(boardSource.includes('zone: "deckInspection" as const'));
  assert.ok(boardSource.includes("setDeckInspection(beginDeckInspectionSession(nextInspection))"));
  ...
  assert.ok(boardStateSource.includes("deckInspection: []"));
  assert.ok(migration.includes("'deckInspection'"));
  assert.ok(migration.includes("when v_zone = 'deckInspection' then p_viewer = v_player and not p_spectator"));
  assert.ok(migration.includes("'hidden-' || v_player || '-' || v_zone || '-'"));
});
...
test("山札確認は縦ダイヤルとMAXを排他利用し、前回枚数と表示順をローカル状態だけで保持する", () => {
  ...
  assert.ok(boardSource.includes('mode: "deck"'));
  assert.equal(boardSource.includes('mode: pending.useMax ? "cost" : "deck"'), false);
  assert.ok(boardSource.includes('cards={board.players[deckInspection.owner].deckInspection}'));
});
