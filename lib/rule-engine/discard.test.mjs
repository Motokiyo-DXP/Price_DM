...
const fixture = () => ({ players: {
  p1: { deck: [card("d1")], hand: [card("h1"), card("h2"), card("h3")], graveyard: [card("g1")], mana: [{ card: card("m1"), tapped: false }] },
  p2: { deck: [card("p2d1")], hand: [card("p2h1")], graveyard: [card("p2g1")], mana: [] },
} });
...
test("DISCARD moves only the selected card to graveyard tail and preserves identity [DISCARD-001]", () => {
  ...
  assert.strictEqual(result.state.players.p1.mana, input.players.p1.mana);
...
test("DISCARD events retain the attempted/result move semantics", () => {
  ...
  assert.equal(Object.hasOwn(result.events[0], "finalDestinationZone"), false);
  assert.equal(result.events[1].finalDestinationZone, "graveyard");
});
