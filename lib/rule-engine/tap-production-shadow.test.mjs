...
test("mismatch and logging failure cannot change the Production TAP", () => {
  ...
  const warnings = [];
  const mismatch = runTapProductionShadow(current, "p1", "p1-m1", options, {
    shadowRunner: () => "MISMATCH",
    warn: (...args) => warnings.push(args),
  });
  assert.deepEqual(mismatch.board, toggleLegacyCardTapState(current, "p1", "mana", "p1-m1", options));
  assert.equal(mismatch.status, "MISMATCH");
  assert.deepEqual(warnings, [["[rule-shadow][TAP] comparison did not strictly match", { player: "p1", status: "MISMATCH" }]]);
