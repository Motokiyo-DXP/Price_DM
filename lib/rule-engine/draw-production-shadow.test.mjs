...
test("production DRAW for p2 returns the Legacy board and a strict Shadow match", () => {
  const input = fixture();
  const legacy = drawRandomCard(input, "p2");
  const warnings = [];
  const result = runDrawProductionShadow(input, "p2", { warn: (...args) => warnings.push(args) });

  assert.deepEqual(result.board, legacy);
  assert.equal(result.status, "STRICT_MATCH");
  assert.deepEqual(warnings, []);
  assert.deepEqual(result.board.players.p2.deck.map(({ instanceId }) => instanceId), ["p2-d2"]);
  assert.deepEqual(result.board.players.p2.hand.map(({ instanceId }) => instanceId), ["p2-h1", "p2-d1"]);
  assert.strictEqual(result.board.players.p1, input.players.p1);
});

test("two sequential Production DRAWs remain strict Legacy matches", () => {
  const input = fixture();
  const first = runDrawProductionShadow(input, "p1");
  const second = runDrawProductionShadow(first.board, "p1");
  const legacy = drawRandomCard(drawRandomCard(input, "p1"), "p1");

  assert.equal(first.status, "STRICT_MATCH");
  assert.equal(second.status, "STRICT_MATCH");
  assert.deepEqual(second.board, legacy);
  assert.deepEqual(second.board.players.p1.deck, []);
  assert.deepEqual(second.board.players.p1.hand.map(({ instanceId }) => instanceId), ["p1-h1", "p1-d1", "p1-d2"]);
});
