...
test("登録画面の短文検索はRLS下でindexableな専用経路を使う", () => {
  const shortSearchMigration = readFileSync(
    new URL("../supabase/migrations/20260919130003_split_registration_short_search_paths.sql", import.meta.url),
    "utf8",
  );

  assert.match(shortSearchMigration, /language plpgsql/);
  assert.match(shortSearchMigration, /search_registration_cards_long/);
  assert.match(shortSearchMigration, /is_ascii_short/);
  assert.match(shortSearchMigration, /card_number_search text generated always/);
  assert.match(shortSearchMigration, /product_name_search text generated always/);
  assert.match(shortSearchMigration, /operator\(pg_catalog\.~>=~\)/);
  assert.match(shortSearchMigration, /card_prints_canonical_card_id_active_idx/);
});
