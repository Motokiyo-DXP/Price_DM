...
test("状況に応じてMaya第一階層を切り替える", () => {
  ...
  const deck = getContextualActions({ face: "face_down", tapped: false, zone: "deck" });
  assert.equal(deck.find(({ slot }) => slot === 2)?.action, "shuffle");
  assert.equal(deck.find(({ slot }) => slot === 4)?.action, "yobinion");
});
