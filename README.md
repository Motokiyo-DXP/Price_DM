...
通常のローカル確認先は <http://localhost:3000> です。起動ログでポート3000を確認してください。
「プレビュー」「確認できるようにして」などの依頼は、明示的なデプロイ指示がない限りこのローカル確認を意味します。
別端末から同じWi-Fi経由で確認する場合だけ、[ローカルプレビュー手順](docs/LOCAL_PREVIEW.md) のポート3001を使用します。
Vercel Previewと本番デプロイは、それぞれ明示的に依頼された場合のみ行います。
...
2026-07-25にプロジェクト管理者から利用許諾を取得した旨の確認を受け、
2026-07-28に許諾範囲内の全件取得・検証・本番投入を完了しました。
継続更新では、許諾回答の対象項目・クレジット・頻度・公開範囲を必ず優先します。

全件取得も画像・カード本文・価格を対象外とし、カード名、機械生成した通常の読み、
収録番号、商品名、公式URLだけを保存します。第三者由来の別名は含めません。1件ずつ1秒間隔で確認するため
