...
`/admin`の「既存店舗の読み・別名を編集」を使用する。店舗名・読み・登録済み別名で候補を絞り込み、対象店舗を選ぶ。絞り込みはひらがな・カタカナ、空白・中点・ハイフン、全角英字の表記差を許容する。

読みは確認できる店舗名の読みを入力する。別名は略称・英字表記・通称など、同じ店舗だと確認できた表記だけを1行に1件入力する。保存後は価格登録画面の店舗検索で想定した表記が一致することを確認する。
